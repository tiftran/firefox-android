...
internal class ReleaseMetricController(
    ...
    internal fun Fact.process(): Unit = when (component to item) {
        ...
        Component.FEATURE_FXSUGGEST to FxSuggestFacts.Items.SPONSORED_SUGGESTION_CLICKED -> run {
            val clickInfo = metadata?.get("clickInfo")?.let {
                it as? FxSuggestClickInfo.Amp
            } ?: return@run

            FxSuggest.pingType.set("fxsuggest-click")
            FxSuggest.source.set("urlbar")
            FxSuggest.matchType.set("firefox-suggest")
